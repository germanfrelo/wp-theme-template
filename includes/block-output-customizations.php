...
function themeslug_apply_block_customizations( $block_content, $block ) {
	...
	if ( isset( $conf['when'] ) ) {
		if ( is_callable( $conf['when'] ) ) {
			$should_apply = (bool) call_user_func( $conf['when'], $block, $block_content );
		} elseif ( is_array( $conf['when'] ) ) {
			if ( isset( $conf['when']['attrs'] ) && is_array( $conf['when']['attrs'] ) ) {
				foreach ( $conf['when']['attrs'] as $attr_key => $attr_val ) {
					if ( ! isset( $block['attrs'][ $attr_key ] ) || $block['attrs'][ $attr_key ] !== $attr_val ) {
						$should_apply = false;
						break;
					}
				}
	...
	$params = isset( $conf['params'] ) && is_array( $conf['params'] ) ? $conf['params'] : [];
...
function themeslug_replace_button_svg( $html, $params ) {
	if ( empty( $params['button_class'] ) || empty( $params['svg'] ) || ! is_array( $params['svg'] ) ) {
		return $html;
	}
...
function themeslug_wrap_select_with_icon( $html, $params ) {
	$wrapper_class = isset( $params['wrapper_class'] ) ? $params['wrapper_class'] : 'select-wrapper';
	$svg_markup    = isset( $params['svg_markup'] ) ? $params['svg_markup'] : '';
	$select_regex  = isset( $params['select_regex'] ) ? $params['select_regex'] : '/(<select[^>]*>.*<\/select>)/sU';
