...
document.addEventListener("DOMContentLoaded", () => {
	...
	const wcAccount = document.querySelector("body.woocommerce-account");

	if (wcAccount) {
		addClassToElements("flow", wcAccount, [
			"form",
			".col-1.u-column1",
			".col-2.u-column2",
		]);

		addClassToElements("stack", wcAccount, [
			".col-1.u-column1",
			".col-2.u-column2",
			".form-row",
		]);

		addClassToElements("switcher", wcAccount, [".u-columns.col2-set"]);
	}
	...
	if (wcAccountLoggedIn) {
		...
		addClassToElements("grid", wcAccountLoggedIn, [
			".woocommerce-MyAccount-navigation > ul",
		]);

		addClassToElements("with-sidebar-left", wcAccountLoggedIn, [
			".woocommerce",
		]);
	}
});
